...
public:
  ...
  // for iTCP
  TracedValue<uint32_t>  m_dupAckCount;
  TracedValue<uint32_t>  m_timeoutCount;
...
public:
  // Neat NN
  NEAT::Network *nn_cwnd;
  // Set true for PktsAck evaluation
  bool evalTime = true;
  bool neatActivated = false;
  /**
   * Get the type ID.
   * \brief Get the type ID.
   * \return the object TypeId
  ...
  /**
   * Set NEAT NN for Socket
   */
  virtual void SetNeat (NEAT::Network* ntw);
  /**
  ...
  */
  ...
  TracedCallback<uint32_t, uint32_t> m_dupAckTrace;
  ...
  /**
  * Callback function for iTCP
  *
  */

  void UpdateDupAckCount (uint32_t oldValue, uint32_t newValue);
  void UpdateTimeoutCount (uint32_t oldValue, uint32_t newValue);
...
protected:
  ...
  uint32_t          m_timeoutCount;    //!< Timeout counter
  ...
  bool              m_activatedItcp = false;    //!< Set Bool to false if iTcp is running 

...
TcpSocketState::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpSocketState")
    ...
    .AddTraceSource ("NextTxSequence",
                     ...
                     "ns3::SequenceNumber32TracedValueCallback")
    .AddTraceSource ("DuplicateAckCount",
                     "Counter for Duplicate ACKs",
                     MakeTraceSourceAccessor (&TcpSocketState::m_dupAckCount),
                     "ns3::TracedValue::Uint32Callback")
    .AddTraceSource ("TimeoutCount",
                     "Counter for timeouts",
                     MakeTraceSourceAccessor (&TcpSocketState::m_timeoutCount),
                     "ns3::TracedValue::Uint32Callback")
  ;
  return tid;
}
...
TcpSocketState::TcpSocketState (const TcpSocketState &other)
  : Object (other),
    m_dupAckCount (other.m_dupAckCount),
    m_timeoutCount (other.m_timeoutCount),
    m_cWnd (other.m_cWnd),
    ...
    m_isFirstPartialAck (true)
{
  ...
  ok = m_tcb->TraceConnectWithoutContext ("DuplicateAckCount",
                                          MakeCallback (&TcpSocketBase::UpdateDupAckCount, this));
  NS_ASSERT (ok == true);

  ok = m_tcb->TraceConnectWithoutContext ("TimeoutCount",
                                          MakeCallback (&TcpSocketBase::UpdateTimeoutCount, this));
  NS_ASSERT (ok == true);
}
...
void
TcpSocketBase::SetNeat (NEAT::Network* ntw)
{
  if (ntw != NULL){
    nn_cwnd = ntw;
    neatActivated = true;
              ...
              if (segsAcked >= oldDupAckCount)
              ...
              else
                {
                 // NS_ASSERT (oldDupAckCount - segsAcked == 1);
                  std::cout << "oldDupAckCount" << std::endl;
                }
...
void 
TcpSocketBase::UpdateDupAckCount (uint32_t oldValue, uint32_t newValue)
{
  m_dupAckTrace (oldValue, newValue);
}

void
TcpSocketBase::UpdateTimeoutCount (uint32_t oldValue, uint32_t newValue)
{
  m_timeoutTrace (oldValue, newValue);
}
void
...
double 
TcpSocketBase::log_base (double x)
{
  if(x<=1)
    return 1;
  double y = 1+(log(x)/log(2));
  return y;
}

double 
TcpSocketBase::negative_linear (double x)
{
  double y = ((x<=0)?x:0);
  return y;
}
...
double
TcpSocketBase::itcp_nn (double cwnd, double t_out, double dupack)
{
  ...
  double i_in = cwnd + t_out*(-3.0)*scale + dupack*(-1.5)*scale;
  double i_out = f_inc(i_in);

  //double d_in = cwnd*wcd + t_out*wtd*scale + dupack*wad*scale;
  double d_in = cwnd + t_out*(-4.0)*scale + dupack*(-2.0)*scale;
  double d_out = f_dec(d_in);

  //double s_in = cwnd*wcs + t_out*wts*scale + dupack*was*scale;
  double s_in = cwnd + t_out*(-2.0)*scale + dupack*(-1.0)*scale;
  double s_out = f_same(s_in);


  //HIDDEN LAYER 2
  double decision_out;
  if( ((i_out)>=(d_out)) && ((i_out)>=(s_out)) )
  {
    decision_out = 1.0;
  }
  else if( ((s_out)>(i_out)) && ((s_out)>=(d_out)) )
  {
    decision_out = 0.0;
  }
  else
  ...
  if(r<0.3 && 0.3 > 0)
  {
    decision_out = (double)(rand()%3) - 1.0;
  }
  ...
  else if(decision_out > 0.0) 
  {
    change_out = 2.0 * i_out ;
    change_out = (change_out>=1)?change_out:1;
    change_out = (change_out<=64)?change_out:64;
  }
  else
  {
    change_out = (cwnd * d_out / 4.0);
  ...
  //OUTPUT LAYER
  double final_cwnd = cwnd + change_out;
...
TcpSocketBase::itcp()
{
  double iTCPmax = 10000;
  bool success = false;
  double out = 0;
  double cwnd = m_tcb->m_cWnd;
  double timeoutCount = m_timeoutCount;
  double dupAckCount = m_dupAckCount;
  double input [3];
  input [0] = cwnd/m_tcb->m_segmentSize;
  input [1] = timeoutCount;
  input [2] = dupAckCount;

  if (nn_cwnd != NULL && neatActivated){
 
  nn_cwnd->load_sensors(input);

  success = nn_cwnd->activate ();

  // get new output
  out= (int) (*(nn_cwnd->outputs.begin()))->activation;

  if (success){
  
  /** SIGMOID ADD **/
  /* double add = m_tcb->m_segmentSize;
   double tmp = 0;
   if (out > 0){
    tmp = m_tcb->m_cWnd + add;
  ...
  /** IDENTITY ACTIVATION FUNCTION ADD **/
  /*  double seg =  m_tcb->m_segmentSize;
    double add = out * seg;
    double tmp = m_tcb->m_cWnd + add;
    if (tmp > 0){
      if(tmp < 40000){
      m_tcb->m_cWnd = tmp;
      } else {
  ...
  /** END IDENTITY ACTIVATION FUNCTION ADD **/
   
  /** IDENTITY ACTIVATION FUNCTION DIRECT **/
    double add = m_tcb->m_segmentSize;
    double tmp = out * add;
    if (tmp > 0){
      if(tmp < 40000){
  ...
  /** END IDENTITY ACTIVATIION FUNCTION DIRECT**/
  } else {
        m_tcb->m_cWnd = 0;
        }
  ...
  /* iTCP implementation */
    ...
    //double res = itcp_nn(m_tcb->m_cWnd/m_tcb->m_segmentSize, m_timeoutCount, m_dupAckCount);
    double res = itcp_nn(m_tcb->m_cWnd, m_timeoutCount, m_dupAckCount);
    double t_res = (double)((int)res);
    res = ((res-t_res)>0)?(t_res+1):t_res;
    res = (res<=iTCPmax)? res:iTCPmax;
    m_tcb->m_cWnd = res * m_tcb->m_segmentSize;
    //m_tcb->m_cWnd = res;
   // uncomment for periodic evaluation
   // Simulator::Schedule (MilliSeconds (10), &TcpSocketBase::itcp, this);
  }
}

void
TcpSocketBase::startItcp (void)
{
 // uncomment for periodic evaluation
 // if (m_activatedItcp == false){
 //   m_activatedItcp = true;
    itcp();
// }
}
